...
func addTensorAnisotropyFrom(dst *data.Slice, M magnetization, Msat, KuXX, KuYY, KuZZ *RegionwiseScalar, AnisUxx, AnisUyy, AnisUzz *RegionwiseVector) {
	if KuXX.nonZero() {
		...
		cuda.AddUniaxialAnisotropy(dst, M.Buffer(), ms, kuXX, uXX)
	}
	if KuYY.nonZero() {
		ms := Msat.MSlice()
		defer ms.Recycle()
		...
		cuda.AddUniaxialAnisotropy(dst, M.Buffer(), ms, kuYY, uYY)
	}
	if KuZZ.nonZero() {
		ms := Msat.MSlice()
		defer ms.Recycle()
		...
		cuda.AddUniaxialAnisotropy(dst, M.Buffer(), ms, kuZZ, uZZ)
	}
}
